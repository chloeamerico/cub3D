...
// /* ==== helpers ==== */
// static uint32_t argb_u32(int a, int r, int g, int b)
// {
...
// /* Convertit char** map -> int** map_int (spaces = mur) */
...
// static void set_player_from_data(t_game *g)
// {
//     /* position au centre de la case */
//     g->px = g->data->player_x + 0.5;
//     g->py = g->data->player_y + 0.5;

//     /* orientation & plan caméra (≈66°) */
//     if (g->data->player_dir == 'N'){ g->dir_x=0;  g->dir_y=-1; g->pl_x= 0.66; g->pl_y=0;    }
//     else if (g->data->player_dir == 'S'){ g->dir_x=0;  g->dir_y= 1; g->pl_x=-0.66; g->pl_y=0;    }
//     else if (g->data->player_dir == 'E'){ g->dir_x=1;  g->dir_y= 0; g->pl_x= 0.0;  g->pl_y=0.66; }
//     else { /* W */                       g->dir_x=-1; g->dir_y= 0; g->pl_x= 0.0;  g->pl_y=-0.66;}

//     /* couleurs depuis config */
//     g->floor_col = argb_u32(255, g->config->F[0], g->config->F[1], g->config->F[2]);
//     g->ceil_col  = argb_u32(255, g->config->C[0], g->config->C[1], g->config->C[2]);
// }
...
//     set_player_from_data(&g);

//     if (!init_mlx(&g) || !frame_new(&g, W, H)) { free_all_local(&g); return 1; }

//     if (!load_textures(&g, g.config->no_texture, g.config->so_texture,
//                            g.config->we_texture, g.config->ea_texture))
//     {
//         free_all_local(&g);
//         return 1;
//     }

//     (void)load_floor_texture(&g, "grass.xpm");
